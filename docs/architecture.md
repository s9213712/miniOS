...
- Stage 3（進行中）：Linux ABI + ELF loader + minimal `execve` bring-up。
- Stage 4（後續）：FS/網路/安全與 Linux-like 能力擴展。
...
- `kernel/core/userproc.c`、`userimg.c`：Linux ABI preview、ELF 載入與 userspace 前置 scaffold。
...
- `run linux-abi`：Linux syscall 編號相容的預覽子集合，不是完整 POSIX。
...
- 真正 userspace entry/return（非僅 dry-run/scaffold）。
- `execve` 最小可執行路徑（segment copy/zero + context + exit 回收）。
