...
- `run linux-abi`：保留較完整的 kernel probe 路徑，會額外驗證 `execve` 教學鏈路，不是完整 POSIX。
- `run hello` / `run ticks`：會先把 kernel 內嵌 blob 複製到 user pages，再以 ring-3 + `syscall` trampoline 執行並返回 kernel。
- `execve("/bin/hello_linux_tiny", ...)`：仍是較完整的 ELF userspace 證據，補足 direct blob 之外的 loader / stack / `argv` 鏈路。
