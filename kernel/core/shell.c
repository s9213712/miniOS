...
static void shell_cmd_capabilities(void) {
    ...
    console_write_string("    - `make test-userimg-loader` validates ELF load layout + user stack mapping into VMM metadata\n");
...
static void shell_exec(const char *line) {
    ...
    if (cmd_len == 7 && shell_streq(trimmed_line, "version")) {
        console_write_string("MiniOS Phase 38 (user image context scaffold)\n");
        return;
    }
...
void shell_run(void) {
    ...
    console_write_string("MiniOS shell (phase 38)\n");
    shell_print_help();
    for (;;) {
        shell_print_prompt();
