...
static void shell_cmd_capabilities(void) {
    ...
    console_write_string("    - user syscall subset: read/write/writev/close/fstat/mmap/munmap/brk/uname/getpid/gettid/set_tid_address/arch_prctl/execve/exit_group/openat/newfstatat\n");
...
static void shell_exec(const char *line) {
    ...
    if (cmd_len == 7 && shell_streq(trimmed_line, "version")) {
        console_write_string("MiniOS Stage 3 (Phase 43: x86-64 syscall entry)\n");
        return;
    }
...
void shell_run(void) {
    ...
    console_write_string("MiniOS shell (stage 3, phase 43)\n");
    shell_print_help();
    for (;;) {
        shell_print_prompt();
