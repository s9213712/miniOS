...
- Added `set_robust_list`/`get_robust_list` bootstrap probes for single-threaded runtime compatibility.
...
- Added an explicit initial exe-path setter so `/proc/self/exe` works before a process calls `execve`.
